...
export const ImportCommand = effectCmd({
  command: "import <file>",
  describe: "import session data from a JSON export file",
  builder: (yargs) =>
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  // [gp] Product: upstream also accepted an opencode-console share URL here, which
  // was fetched with the account token. With no first-party account there is nothing
  // to authenticate against, so a URL is rejected with a reason instead of a 401.
  if (file.startsWith("http://") || file.startsWith("https://")) {
    process.stdout.write(
      "Console share links are not supported: this harness has no opencode account and does not share sessions to a cloud service." +
        EOL +
        "Export the session to a JSON file locally and import that file instead." +
        EOL,
    )
    return
  }
