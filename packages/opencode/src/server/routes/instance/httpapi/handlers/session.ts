...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionHttpApi.create")(function* (ctx: { payload?: Session.CreateInput }) {
      // [gp] Product: upstream created the session through the share service so that
      // `share: "auto"` could upload it. There is no sharing here, so this is the
      // session service — the auto-share configuration went with it.
