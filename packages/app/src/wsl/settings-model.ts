...
export function wslOpencodeAction(check?: WslOpencodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installHarness"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateHarness"
}
...
function addServerDistroStatus(input: {
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateHarness" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.harnessMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installHarness" }, tone: "warning" }
  ...
  if (!addServerOpencodeReady(input.opencodeCheck)) {
    ...
    return {
      ...
      label: installingOpencode
        ? { key: "wsl.onboarding.updatingHarness" }
        : update
          ? { key: "wsl.onboarding.updateHarness" }
          : { key: "wsl.onboarding.installHarness" },
