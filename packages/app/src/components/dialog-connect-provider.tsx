...
function ProviderPickerV2(props: {
  ...
  // [gp] Product: the first-party `opencode` / `opencode-go` providers are not in
  // the catalog any more (they were gateways to a paid cloud behind an account this
  // product does not have), so they are not featured. Everything here is a
  // third-party provider unlocked by the user's own API key.
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          {/* [gp] Product: the first-party gateway upsell is gone with the provider;
              every provider gets the same bring-your-own-key explanation. */}
