...
/**
 * The product's environment-variable prefix, and the single place a variable is
 * resolved.
 *
 * WHY A RESOLVER AND NOT A RENAME. Every knob this product has was spelled
 * `OPENCODE_*` upstream, and a person who already exported one of them must keep
 * working — a private-isation that silently ignores an existing setting is its own
 * bug. So: the product name is read first, the upstream name second, and both are
 * accepted forever. `Flag.OPENCODE_*` keys stay as internal property names; what
 * changed is the *name a user types*.
 */
const PRODUCT_PREFIX = "GLASSPANE_HARNESS_"

/** Read an env var by its product name, falling back to the legacy upstream name. */
export function read(key: string) {
  if (key.startsWith("OPENCODE_")) {
    const product = PRODUCT_PREFIX + key.slice("OPENCODE_".length)
...
export const Flag = {
  ...
  // Experimental
  get OPENCODE_EXPERIMENTAL_FILEWATCHER() {
    return Config.succeed(truthy("OPENCODE_EXPERIMENTAL_FILEWATCHER"))
  },
  get OPENCODE_EXPERIMENTAL_DISABLE_FILEWATCHER() {
    return Config.succeed(truthy("OPENCODE_EXPERIMENTAL_DISABLE_FILEWATCHER"))
  },
