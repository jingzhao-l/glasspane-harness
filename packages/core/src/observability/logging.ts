...
export function fileLogger(file = path.join(Global.Path.log, "glasspane-harness.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = EnvRead("OPENCODE_LOG_LEVEL")?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return EnvRead("OPENCODE_PRINT_LOGS") === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
